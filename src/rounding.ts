...
export class RoundingConfig {
   ...
   */
  private static _defaultRoundingMode: RoundingMode = RoundingMode.NONE;
...
export function round(
  ...
  switch (mode) {
    ...
    case RoundingMode.DOWN:
      roundedScaledValue =
        scaledValue >= 0 ? Math.floor(scaledValue) : Math.ceil(scaledValue);
      break;
    case RoundingMode.UP:
      roundedScaledValue =
        scaledValue >= 0 ? Math.ceil(scaledValue) : Math.floor(scaledValue);
      break;
    case RoundingMode.HALF_UP:
      ...
      const fractionHalfUp =
        Math.abs(scaledValue) - Math.floor(Math.abs(scaledValue));
      if (fractionHalfUp < 0.5 - epsilon) {
    ...
    case RoundingMode.HALF_EVEN:
      ...
      if (fraction === 0.5) {
        ...
        roundedScaledValue =
          scaledValue < 0 ? -roundedScaledValue : roundedScaledValue;
      } else {
